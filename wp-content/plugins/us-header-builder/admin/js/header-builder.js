...
!function($){
	...
	$.extend($ushb.HTemplates.prototype, $usof.mixins.Events, {
		init: function(){
			...
			this._events = {
				select: function(event){
					var $item = $(event.target).closest('.us-hb-window-item');
					if (!confirm($ushb.instance.translations['template_replace_confirm'])) return;
					this.hide();
	...
	$usof.field['header_builder'] = {
		...
		init: function(options){
			...
			var $usofSaveBtn = $('.usof-header .usof-control:first');
			this.$toolbarBtn = this.$container.find('.usof-control.for_htemplates, .usof-control.for_help')
				.insertAfter($usofSaveBtn);
			this.on('beforeShow', function(){
				this.$toolbarBtn.insertAfter($usofSaveBtn);
			}.bind(this));
			this.on('beforeHide', function(){
				this.$toolbarBtn.detach();
			}.bind(this));
			this.$toolbarBtn.filter('.for_htemplates').on('click', this._showTemplatesBtnClick.bind(this));
		...
		_addBtnClick: function(event){
			...
			$ushb.elist.off('beforeShow').on('beforeShow', function(){
				$ushb.elist.$container
					.toggleClass('hide_menu', this.value.data['menu:1'] !== undefined)
		...
		_updateElementPlaceholder: function($elm, id, values){
			...
			if (type == 'text' && (values.text || values.icon)) {
				...
				content += this.translations['main_menu'];
		...
		getValue: function(){
			...
			this.$input.each(function(index, input){
				var $input = $(input),
					...
					rawValue = $input.val();
				if (name == 'hide_for_sticky') return;
