...
if ( empty( $value ) OR ! is_array( $value ) ) {
	...
	$value = $us_header_settings;
}
$value = us_fix_header_settings( $value );

$output = '<div class="us-hb">';

// States
$output .= '<div class="us-hb-states">';
$output .= '<div class="us-hb-state for_default active">' . __( 'Default', 'us' ) . '</div>';
...
foreach ( $hb_options_sections as $hb_section => $hb_section_title ) {
	...
	$output .= '<div class="us-hb-options-section-content">';
	foreach ( us_config( 'header-settings.options.' . $hb_section, array() ) as $field_name => $fld ) {
		if ( ! isset( $fld['type'] ) ) {
			continue;
		}
		$field_html = us_get_template( 'vendor/usof/templates/field', array(
			'name' => $field_name,
...
$output .= '<div class="us-hb-params"';
$output .= us_pass_data_to_js( array(
	'navMenus' => us_get_nav_menus(),
	// TODO Default values
) );
$output .= '></div>';
$output .= '<div class="us-hb-value"' . us_pass_data_to_js( $value ) . '></div>';
...
$output .= '<div class="us-hb-defaults"' . us_pass_data_to_js( $elms_defaults ) . '></div>';
$translations = array(
	...
	'main_menu' => __( 'Main Menu', 'us' ),
	'additional_menu' => __( 'Additional Menu', 'us' ),
	'dropdown' => __( 'Dropdown', 'us' ),
	'social_links' => __( 'Social Links', 'us' ),
	'button' => __( 'Button', 'us' ),
	'cart' => __( 'Cart', 'us' ),
);
$output .= '<div class="us-hb-translations"' . us_pass_data_to_js( $translations ) . '></div>';

$output .= '</div>';

$output .= '<div class="us-hb-screenlock"><div>' . sprintf( __( '<a href="%s">Activate the theme</a> to unlock Header Builder', 'us' ), admin_url( 'admin.php?page=us-home' ) ) . '</div></div>';
...
$output .= us_get_template( 'templates/ebuilder', array(
	'titles' => $elms_titles,
	'body' => '',
) );

// Empty header templates window for loading the templates afterwards
$output .= us_get_template( 'templates/htemplates', array(
	'body' => '',
) );

$output .= '<div class="usof-control for_htemplates"><button class="usof-button type_htemplates"><span>' . __( 'Header Templates', 'us' ) . '</span></button></div>';
$output .= '<div class="usof-control for_help"><a href="https://help.us-themes.com/' . strtolower( US_THEMENAME ) . '/hb/" target="_blank"></a></div>';
