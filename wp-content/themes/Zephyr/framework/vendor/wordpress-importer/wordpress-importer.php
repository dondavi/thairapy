	...
	class WP_Import extends WP_Importer {
		...
		function process_posts() {
			...
			foreach ( $this->posts as $post ) {
				...
				if ( ! post_type_exists( $post['post_type'] ) ) {
					printf(
						'Failed to import &#8220;%s&#8221;: Invalid post type %s', esc_html( $post['post_title'] ), esc_html( $post['post_type'] )
					);
					echo '<br />';
				...
				if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
					...
					if ( is_wp_error( $post_id ) ) {
						printf(
							'Failed to import %s &#8220;%s&#8221;', $post_type_object->labels->singular_name, esc_html( $post['post_title'] )
						);
						if ( defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG ) {
		...
		function process_attachment( $post, $url ) {
			if ( ! $this->fetch_attachments ) {
				return new WP_Error(
					'attachment_processing_error', 'Fetching attachments is not enabled'
				);
			}
...
function wordpress_importer_init() {
	...
	register_importer(
		'wordpress', 'WordPress', 'Import <strong>posts, pages, comments, custom fields, categories, and tags</strong> from a WordPress export file.', array(
		$GLOBALS['wp_import'],
		'dispatch',
	)
	);
